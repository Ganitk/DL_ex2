    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_dropout and i!= 0:
                h, caches['drop' + str(i+1)] = dropout_forward(h, self.dropout_param) 
            if self.normalization == 'batchnorm':
                gamma = self.params['gamma'+str(i+1)]
                beta = self.params['beta'+str(i+1)]
                h, caches[i+1] = affine_bn_relu_forward(h, w, b, gamma,
                                beta, self.bn_params[i])
            else:
                h, caches[i+1] = affine_relu_forward(h, w, b)
        i = self.num_layers - 1
        if self.use_dropout:
            h, caches['drop' + str(i+1)] = dropout_forward(h, self.dropout_param) 

        scores, caches[self.num_layers] = affine_forward(h, self.params['W'+str(i+1)],
        ...
        dh, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] =affine_backward(dscores, caches[self.num_layers])
        grads['W' + str(self.num_layers)] += self.reg * self.params['W'+str(self.num_layers)]
        
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout and i!=0:
                dh = dropout_backward(dh, caches['drop' + str(i + 1)])
            if self.normalization == 'batchnorm':
                dh, grads['W'+str(i)], grads['b'+str(i)], grads['gamma' + str(i)],\
                grads['beta' + str(i)] = affine_bn_relu_backward(dh, caches[i])
            else:
